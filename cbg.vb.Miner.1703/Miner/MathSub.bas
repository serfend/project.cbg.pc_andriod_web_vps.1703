...
Public Function max(ParamArray parm()) As Variant
    Dim i As Long
    For i = 0 To UBound(parm)
        If max < parm(i) Then max = parm(i)
    Next
End Function
Public Function min(ParamArray parm()) As Variant
    Dim i As Long
    min = parm(0)
    For i = 1 To UBound(parm)
        If min < parm(i) Then min = parm(i)
    Next
End Function
