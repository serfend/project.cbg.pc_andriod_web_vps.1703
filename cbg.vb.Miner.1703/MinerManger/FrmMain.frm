...
On Error GoTo Err:
    Dim nextRefreshTimeCoreLoad As Long
    nextRefreshTimeCoreLoad = GetTickCount - nextCoreLoadTime
                ...
                If thisThreadId <> "" Then
                    ...
                    Else
                        ...
                        If thisThread.needNewIP Then
                            ...
                            thisThread.ip = GetSingleIp(thisThread)
                        End If
...
On Error GoTo Err:
    If LstIpShow.ListItems.Count < 5 Then
        Dim http As HttpGetter
        Set http = New HttpGetter
        Dim newInfo As String
        newInfo = http.getHtml("http://ip.11jsq.com/index.php/api/entry?method=proxyServer.generate_api_url&packid=2&fa=0&fetch_key=&qty=5&time=1&pro=&city=&port=1&format=txt&ss=1&css=&dt=1&specialTxt=3&specialJson=")
        Dim cstrs As CString
        Set cstrs = New CString
        cstrs.Append newInfo
        If InStr(1, newInfo, "白名单") Then
            MsgBox "加载ip失败:" & newInfo
            End
        End If
        Debug.Print newInfo
        Dim ipList() As String
        'ipList = cstrs.GetAllElement("ip"":""", "}")
...
Err:
    ...
    If MsgBox("继续运行?", vbYesNo) = vbNo Then End
End Sub
Private Function GetSingleIp(thread As mainCore) As String
On Error GoTo Err:
