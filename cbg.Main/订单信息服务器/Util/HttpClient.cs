...
namespace DotNet4.Utilities.UtilHttp
{
	public class HttpApi : WinHttp.WinHttpRequestClass
	{
		...
		public HttpApi(bool UsedFidder,string UsedProxy) : this()
		{
			if (UsedFidder) this.SetProxy(2, "127.0.0.1:8888",null);
			else if (UsedProxy != null) this.SetProxy(2, UsedProxy,null);
		}
	...
	class HttpClient
	{
		...
		public static string UsedProxy { get; set; }
		...
		public HttpClientChild GetHtml(string url = null, string method = null, string postData = null, string userAgent = null, string host = null, string referer = null, bool ifModifies = true, int timeOut = 30000, string clientId = null, Action<HttpApiEvent.HttpDocument> callBack = null)
		{
			...
			var child = new HttpClientChild(this, HttpClient.UsedFidder,HttpClient.UsedProxy)
			{
				ID = clientId
			};
	...
	class HttpClientChild
	{
		...
		private readonly HttpClient parent;
		...
		public HttpClientChild(HttpClient parent, bool UsedFidder = false,string UsedProxy=null)
		{
			this.parent = parent;
			http = new HttpApi(UsedFidder, UsedProxy);
			http.OnHttpError += (x) =>
			{
		...
		public bool GetResponse(HttpItem item = null, bool syn = false, Action<HttpApiEvent.HttpDocument> CallBack = null)
		{
			...
			//try
			{
				...
				else
				{
					...
					{
						http.OnHttpError.Invoke(new ErrorEventDelegate(101,ex.Message));
						http.OnDocumentReady.Invoke(new HttpApiEvent.DocumentReady(new byte[0]));
					}
				}

				if (syn) document = null;
				else
				{
	...
	public class HttpContentItem
	{
		...
		{
			return Data == null?null: coding.GetString(Data.ToArray());
		}
		/// <summary>
