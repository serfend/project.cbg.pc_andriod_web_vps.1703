...
namespace DotNet4.Utilities
{
	namespace UtilReg
	{
		...
		public class Reg
		{
			...
			/// </summary>
			public Reg():this(Application.ProductName)
			{
		...
		public static class RegUtil
		{
			...
			{
				return new Reg("sfMinerDigger").In("Main").In(System.Reflection.Assembly.GetExecutingAssembly().GetName().FullName).In("Setting").In("FromLocation").In(frm.Name);
			}
