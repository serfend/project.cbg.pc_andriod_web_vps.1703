(function() {
	...
	function localInitConfig(a, b, c) {
		console.log("z(a,b,c) is calling");
		function d() {
		...
		//目的是将此文件插入到头脚本中  var k = document.getElementsByTagName("script")[0] || document.head;
		var	h, l;
		...
		g = (new Date).getTime();
		a += (~a.indexOf("?") ? "\x26" : "?") + f + "\x3d" + encodeURIComponent(e) + "\x26t\x3d" + g;
		a = a.replace("?\x26", "?");
		var h={};//h = document.createElement("script");
		h.src = a;
		console.log("h.src="+a);
		var info=console.getHttpContent(a);
		info="var httpMsg="+info.substring(info.indexOf("(")+1,info.indexOf("})"))+"}";
		eval(info);
		window.ydConfig=httpMsg.result;
		console.log("json load:"+window.ydConfig.conf);
	...
	function B(a) {
		try {
			console.log("B(a) is calling 将调用ydConfigArr:"+a);
			return window.ydConfigArr;//直接返回全部元素的组合
			// var b = window.ydConfigArr.split(q);
			// if (+b.splice(-1) >= v()) return b.join(q);
			// localStorage.removeItem(a);
			// return ""
		} catch (c) {
			return "B(a) is on exception()";
		}
	}
	function C(URI, successCallBack) {
		console.log("C(a,b) is calling");
		var url = URI.pn,
			d = URI.protocol,
			timeoutCallBack = URI.timeout,
			e = URI.__serverConfig__;
		void 0 === e && (e = {});
		url = u(d, e.configServer || "ac.dun.163yun.com", "/v2/config/js?pn\x3d" + url);
		localInitConfig(url, {
	...
	var F = function(a, b, c) {
			//作用是加载watchman.min.js
			console.InitWatchMan();
			console.log("返回tool.min.js F()");
			c(null,a);
		...
		w = function(a) {
			...
			function b(a, b) {
				...
				var r = b.split(","),
					...
					console.log("加载目标js文件地址 c="+c);
				F(c, {
					charset: "UTF-8",
					attrs: l
				}, function(a) {
					console.log("开始调用Watchman()");
					if (a) return d("[NEWatchman] load js file error");
					a = m.pn;
					var b = m.onload,
						c = m.merged,
						e = D(new window.Watchman(m)),
						f = window.initWatchman.__instances__;
						console.log("initWatchman done.");
			...
			d ? b(a, d) : C(a, function(d, e) {
				...
				console.log("load watchman config ...");
				if (e && 200 === e.code) {
					var k = e.result,
						g = k.ivp,
					...
					try {
						var h = v(g);
						console.log("localStorage.setItem(c, k + q + h):" + c + "=\n"+ k+q+h);//localStorage.setItem(c, k + q + h);
						window.ydConfigArr=k+q+h;//直接赋值
					} catch (l) {}
					b(a, k)
